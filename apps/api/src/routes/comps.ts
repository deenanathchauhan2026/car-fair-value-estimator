...
export async function compsRoutes(app: FastifyInstance) {
  app.post('/api/comps/lookup', async (request, reply) => {
    const parsed = compsLookupSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: 'Invalid comps lookup request', issues: parsed.error.flatten() });
