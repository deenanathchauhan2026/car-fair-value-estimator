...
export class CompsLookupService {
    async lookup(input) {
        ...
        for (const [source, scraper] of [
            ...
            fresh.push(...results);
            if (rankAndFilterComparables(vehicle, [...fresh, ...cachedDb], 8).length >= 6)
                break;
        }
    ...
    findRecentStored(input, limit) {
        ...
        LIMIT @limit
      `).all({ make: input.make, model: input.model, year: input.year ?? null, minYear: input.year ? input.year - 1 : null, maxYear: input.year ? input.year + 1 : null, limit });
...
function sourceOf(comp) {
    const source = comp.rawJson?.source;
    if (source === 'craigslist' || source === 'truecar' || source === 'autolist')
        return source;
