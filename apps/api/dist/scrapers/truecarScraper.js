...
export async function scrapeTrueCar(vehicle) {
...
export function parseTrueCarHtml(html, vehicle) {
    const payloads = extractJsonPayloads(html);
    const listings = [];
    for (const payload of payloads)
        collectListings(payload, vehicle, listings);
    return listings.slice(0, 12);
}
function buildTrueCarUrl(vehicle) {
...
function collectListings(node, vehicle, out) {
    if (!node || out.length >= 12)
        return;
    if (Array.isArray(node)) {
        for (const item of node)
            collectListings(item, vehicle, out);
        return;
    }
    ...
    const price = number(obj.price) ?? number(obj.listPrice) ?? number(obj.amount) ?? number(obj.offers?.price);
    ...
    for (const value of Object.values(obj))
        collectListings(value, vehicle, out);
}
const slug = (value) => value?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') ?? '';
const string = (value) => typeof value === 'string' && value.trim() ? value.trim() : undefined;
const number = (value) => typeof value === 'number' ? value : typeof value === 'string' ? Number(value.replace(/[^0-9.]/g, '')) || undefined : undefined;
